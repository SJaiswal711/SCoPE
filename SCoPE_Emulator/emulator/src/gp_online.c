...
GPModel* online_gp_init(int n_inputs) {
    ...
    gp->sigma_f = 1.0;
    gp->sigma_n = 0.01;
    for (int i = 0; i < n_inputs; i++) gp->ell[i] = 1.0;
    gp->y_mean = 0.0;
    gp->y_std = 1.0;
    return gp;
}
...
void online_gp_train(GPModel *gp, const double *X, const double *y, int N) {
    ...
    for (int j = 0; j < n_in; j++) {
        ...
        var /= N;
        gp->X_std[j] = sqrt(var);
        if (gp->X_std[j] < EPS) gp->X_std[j] = 1.0;
    }
    ...
    if (gp->X_norm) free(gp->X_norm);
    if (gp->y_norm) free(gp->y_norm);
    if (gp->alpha) free(gp->alpha);
    if (gp->chol_L) free(gp->chol_L);
    gp->X_norm = X_norm;
    gp->y_norm = y_norm;
    gp->alpha = alpha;
    gp->chol_L = K;
...
double online_gp_predict_mean(const GPModel *gp, const double *x) {
    if (!gp || gp->n_train == 0 || !gp->alpha) return gp->y_mean;
    int n_in = gp->n_inputs;
    ...
    for (int i = 0; i < gp->n_train; i++) mean_norm += kstar[i] * gp->alpha[i];
    free(x_norm);
    free(kstar);
...
void online_gp_predict(const GPModel *gp, const double *x, double *mean, double *var) {
    ...
    for (int i = 0; i < gp->n_train; i++) {
        double sum = 0.0;
        for (int j = 0; j < i; j++) sum += gp->chol_L[i * gp->n_train + j] * v[j];
        v[i] = (v[i] - sum) / gp->chol_L[i * gp->n_train + i];
    }
    double vv = 0.0;
    for (int i = 0; i < gp->n_train; i++) vv += v[i] * v[i];
    var_norm -= vv;
    if (var_norm < 0.0) var_norm = 0.0;
