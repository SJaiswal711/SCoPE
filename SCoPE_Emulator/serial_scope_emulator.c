 ...
 * Features:
 *   - Emulator trained once after enough true evaluations.
...
/* ---------- MultiGaussian functions (unchanged) ---------- */
...
/* ======================= NEW: Likelihood functions with emulator ======================= */

/* Global emulator instance */
static Emulator *g_emulator = NULL;
static int g_step = 0;                     /* MCMC step counter (for emulator training) */
static int g_use_emulator = 0;             /* Flag to use emulator for prediction */

/* ----------------------------------------------------------------------
   True model evaluation (adds point to buffer)
   ---------------------------------------------------------------------- */
static double compute_loglike_true(double *params) {
    /* Allocate on heap – ownership will be transferred to buffer */
    double *cl_tt = malloc(2601 * sizeof(double));
    double *cl_te = malloc(2601 * sizeof(double));
    double *cl_ee = malloc(2601 * sizeof(double));
    double *cl_bb = malloc(2601 * sizeof(double));
    ...
    /* Update training buffer (the buffer now owns the Cl arrays) */
    g_step++;
    emulator_update(g_emulator, g_step, params, cl_tt, cl_te, cl_ee, cl_bb, logL);
...
static double compute_loglike_emu(double *params) {
    double cl_tt[2601], cl_te[2601], cl_ee[2601], cl_bb[2601];
    double uncertainty;
    if (!emulator_predict(g_emulator, params, cl_tt, cl_te, cl_ee, cl_bb, &uncertainty)) {
        /* Emulator failed – fall back to true model */
        return compute_loglike_true(params);
    }
    /* If uncertainty too high, use true model and collect new data */
    if (uncertainty > 0.1) {   /* 0.1 is a tunable threshold */
        return compute_loglike_true(params);
    }
    /* Otherwise use emulator prediction */
    double chi2 = run_plc(0, params, cl_tt, cl_te, cl_ee, cl_bb);
    return -0.5 * chi2;
}

/* ----------------------------------------------------------------------
   Wrapper used by the MCMC proposal loop – decides which likelihood to call
   ---------------------------------------------------------------------- */
...
int main(int argc, char **argv) {
    ...
    if (!g_emulator) {
        ...
        printf("Emulator initialised. Buffer capacity = %d, max PCA modes = %d\n",
    ...
    /* Training and retraining control */
    ...
    for (int step = 0; step < max_steps; step++) {
        ...
        /* Switch to emulator mode after initial training and after use_emulator_after steps */
        if (initially_trained && step >= emu_cfg.use_emulator_after && emulator_is_ready(g_emulator)) {
            g_use_emulator = 1;
        }

        /* Periodic retraining (if emulator is active) */
        if (g_use_emulator && step - last_retrain_step >= retrain_interval) {
            int new_points = emulator_buffer_size(g_emulator) - emu_cfg.min_train_points;
            if (new_points >= retrain_threshold) {
                printf("Retraining emulator at step %d (new points: %d)...\n", step, new_points);
                emulator_train(g_emulator);
