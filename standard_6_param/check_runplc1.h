...
int initialize_clik_objects(error **err) {
    ...
    // Initialize CAMspec
    cache->camspec = clik_init("/scratch/shambhavij.sps.iitmandi/SCoPE/data/CAMspec_v6.2TN_2013_02_26_dist.clik/", err);
    if (isError(*err)) {
    ...
    cache->commander = clik_init("/scratch/shambhavij.sps.iitmandi/SCoPE/data/commander_v4.1_lm49.clik/", err);
    if (isError(*err)) {
    ...
    // Initialize LowLike
    cache->lowlike = clik_init("/scratch/shambhavij.sps.iitmandi/SCoPE/data/lowlike_v222.clik", err);
    if (isError(*err)) {
