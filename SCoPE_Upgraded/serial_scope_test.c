/*
 * serial_scope.c - Serial MCMC with multiplicity logging
 * Saves all proposals (accepted/rejected) to proposals.bin for offline testing.
 * Each line in chain file: step mult logL param_0 ... param_N
 * Binary format: header (2 ints) then records (step, params, accepted, logL, cl_tt, cl_te, cl_ee, cl_bb)
 * 
 * Usage: ./serial_scope [prefix]
 * Example: ./serial_scope test1_
 *          produces: test1_chain.txt, test1_proposals.bin
 */
...
#define N_SPECTRA 2601   /* number of multipoles */
...
static int ADAPTATION_INTERVAL = 100;
...
/* ---------- likelihood functions (with CL output) ---------- */
double compute_loglike_with_cls(double *params,
...
/* ---------- save proposals to binary file ---------- */
FILE *g_prop_file = NULL;
int g_save_proposals = 0;  /* 0 = don't save (before burn-in), 1 = save (after burn-in) */

void save_proposal_data(int step, double *params, int accepted, double logL,
                        double *cl_tt, double *cl_te,
                        double *cl_ee, double *cl_bb)
{
    if (!g_prop_file || !g_save_proposals) return;
    fwrite(&step, sizeof(int), 1, g_prop_file);
    fwrite(params, sizeof(double), PARAMETERS, g_prop_file);
    fwrite(&accepted, sizeof(int), 1, g_prop_file);
    fwrite(&logL, sizeof(double), 1, g_prop_file);
    fwrite(cl_tt, sizeof(double), N_SPECTRA, g_prop_file);
    fwrite(cl_te, sizeof(double), N_SPECTRA, g_prop_file);
    fwrite(cl_ee, sizeof(double), N_SPECTRA, g_prop_file);
    fwrite(cl_bb, sizeof(double), N_SPECTRA, g_prop_file);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1) {
        strncpy(g_prefix, argv[1], sizeof(g_prefix) - 1);
        g_prefix[sizeof(g_prefix) - 1] = '\0';
        printf("Using prefix: '%s'\n", g_prefix);
    } else {
        printf("No prefix provided. Using default filenames.\n");
    }

    /* ---------- Build filenames with prefix ---------- */
    char chain_filename[512], prop_filename[512];
    snprintf(chain_filename, sizeof(chain_filename), "%schain.txt", g_prefix);
    snprintf(prop_filename, sizeof(prop_filename), "%sproposals.bin", g_prefix);

    /* ---------- Open binary proposals file ---------- */
    g_prop_file = fopen(prop_filename, "wb");
    if (!g_prop_file) {
        fprintf(stderr, "Error: cannot open %s\n", prop_filename);
        exit(1);
    }
    /* Write header: number of parameters and N_SPECTRA */
    fwrite(&PARAMETERS, sizeof(int), 1, g_prop_file);
    int n_spectra = N_SPECTRA;
    fwrite(&n_spectra, sizeof(int), 1, g_prop_file);
    ...
    double init_cl_tt[N_SPECTRA], init_cl_te[N_SPECTRA], init_cl_ee[N_SPECTRA], init_cl_bb[N_SPECTRA];
    double current_loglike = compute_loglike_with_cls(current, init_cl_tt, init_cl_te, init_cl_ee, init_cl_bb);

    /* Save initial point ONLY if burn-in is 0 */
    int burnin = 1000;
    ...
    if (burnin == 0) {
        g_save_proposals = 1;
        save_proposal_data(0, current, 1, current_loglike,
                           init_cl_tt, init_cl_te, init_cl_ee, init_cl_bb);
    } else {
        printf("Burn-in set to %d. Proposals will be saved after step %d.\n", burnin, burnin);
    }
    ...
    /* Open output chain file (text) */
    FILE *chain_file = fopen(chain_filename, "w");
    if (!chain_file) {
        fprintf(stderr, "Error: cannot open chain file\n");
        exit(1);
    }
    fprintf(chain_file, "# step mult logL");
    for (int i = 0; i < PARAMETERS; i++)
    ...
    printf("Burn-in = %d steps. Will save proposals after step %d.\n", burnin, burnin);
    ...
    for (int step = 1; step <= max_steps; step++) {
        ...
        do {
            ...
            prop_loglike = compute_loglike_with_cls(next.f, cl_tt, cl_te, cl_ee, cl_bb);
        ...
        if (isfinite(prop_loglike) && (log_ratio > 0 || exp(log_ratio) > (rand()/(double)RAND_MAX))) {
            accepted = 1;
            /* Write previous point with multiplicity before moving */
            fprintf(chain_file, "%d %d %.12e", step-1, multiplicity, current_loglike);
            for (int i = 0; i < PARAMETERS; i++)
                fprintf(chain_file, " %.12e", current[i]);
            fprintf(chain_file, "\n");
            fflush(chain_file);
            ...
            multiplicity++;   // chain stayed at same point
        }

        /* ---------- Save proposal only after burn-in ---------- */
        if (step >= burnin) {
            g_save_proposals = 1;
            save_proposal_data(step, next.f, accepted, prop_loglike,
                               cl_tt, cl_te, cl_ee, cl_bb);
        } else {
            g_save_proposals = 0;  // still before burn-in
        }
        ...
        /* Progress output */
        if (step % 10 == 0) {
            printf("Step %d, logL = %.2f, factor = %.4f, acc = %d, acc_rate = %.2f, rej = %d, mult=%d, saving=%d\n",
                   step, current_loglike, EntireFactor, accepted, acc_rate, 
                   consecutive_rejections, multiplicity, g_save_proposals);
            fflush(stdout);
        }
    }

done:
    /* Write final point with its multiplicity */
    fprintf(chain_file, "%d %d %.12e", max_steps, multiplicity, current_loglike);
    for (int i = 0; i < PARAMETERS; i++)
        fprintf(chain_file, " %.12e", current[i]);
    fprintf(chain_file, "\n");
    fflush(chain_file);
    ...
    printf("Chain written to %s\n", chain_filename);
    printf("Proposals (including spectra) written to %s\n", prop_filename);
    printf("Total proposals saved: %d (after burn-in of %d steps)\n", 
           (max_steps - burnin > 0) ? max_steps - burnin : 0, burnin);
