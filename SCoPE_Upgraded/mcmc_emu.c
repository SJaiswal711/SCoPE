...
/* Global variable for number of cosmological parameters (set dynamically) */
int g_n_cosmo = 0;
static int mapping_done = 0;

/* Emulator instance for each slave process */
...
//-------------------------------------------------------------------------------------
static unsigned int MULTIPURPOSE_REQUIRED = 2;  // We need one extra slot for MCMC step number

// Define position for MCMC step number in the task array
#define STEP_POS (RANDPOS + PARAMETERS + 1)
...
const unsigned int MIN_SIZE_FOR_FREEZE_IN = 50000;
...
int master(double **startnum, unsigned short int Restart, const char *outdir)
{
  ...
  // Use outdir for all output files
  for (int k = 1; k <= CHAINS; k++) {
    snprintf(path, sizeof(path), "%s/%s_%d.d", outdir, montecarlo_name, k);
    data[k-1] = fopen(path, "a+");
    snprintf(path, sizeof(path), "%s/%s_%d.d", outdir, head_name, k);
    head[k-1] = fopen(path, "a+");
    snprintf(path, sizeof(path), "%s/%s_%d.d", outdir, investigated_name, k);
    investigated[k-1] = fopen(path, "a+");
  }

  snprintf(path, sizeof(path), "%s/progress.txt", outdir);
  FILE *progress = fopen(path, "a+");
  snprintf(path, sizeof(path), "%s/gelmanRubin.txt", outdir);
  FILE *gelmanRubin = fopen(path, "a+");
  snprintf(path, sizeof(path), "%s/covMatrix.txt", outdir);
  ...
  do
  {
    ...
    if (status.MPI_TAG == GIVEMETASK)
    {
      ...
      // Determine number of proposals to generate: if DR is off, only send one (to the middleman)
      ...
      for (int ichains = 0; ichains < num_proposals; ichains++)
      {
        ...
        // Use USE_DR to set ADAPTIVEPOS (1 = DR enabled, 0 = standard MH)
        next[i]->f[ADAPTIVEPOS] = (double)USE_DR;
        next[i]->f[TAKEPOS] = (double)USE_DR;  // not used in slave
        next[i]->f[STEP_POS] = (double)global_step;   // store MCMC step number
    ...
    if (status.MPI_TAG == TAKERESULT) // the slave sent us a result
    {
      ...
      if (take == 1)
        ...
        chainTotalPerformed[i] += (USE_DR ? SLAVEPARCHAIN : 1);  // only one proposal if DR off
      ...
      {
        chain[i][chainBack[i] - 1].Multiplicity += (USE_DR ? SLAVEPARCHAIN : 1);       
        chain[i][chainBack[i] - 1].ReallyInvestigated += (USE_DR ? SLAVEPARCHAIN : 1);
      }
      ...
      if ((min_size > RMIN_POINTS) && (posRnd(1.0) > 0.95))
      {
        ...
        {
          W[k] /= (M * (N - 1.0));
          // Correct Gelman-Rubin formula: R = ((N-1)/N)*W + B/N
          R[k] = (N - 1.0) / N * W[k] + B[k] / N;
          R[k] /= W[k];
        }
        ...
        if (FREEZE_IN == 1 && min_size > MIN_SIZE_FOR_FREEZE_IN)
        {
          ...
          if (ConvergenceReached == 1)
          {
            ...
            drfactor = 1.0;
            FILE *final;
            snprintf(path, sizeof(path), "%s/freezeInEigenvectors.txt", outdir);
            final = fopen(path, "a+");

            // FIX: use snprintf directly, no montecarlo_name1
            for (unsigned int k = 1; k <= CHAINS; k++)
            {
              fclose(data[k - 1]);
              snprintf(path, sizeof(path), "%s/%s_freeze_%d.d", outdir, montecarlo_name, k);
              data[k - 1] = fopen(path, "a+");
            }
            ...
            for (unsigned int j = 0; j < CHAINS; j++)
            {
              ...
              // Removed arbitrary cap: if (EntireFactor[j] > 1.0) EntireFactor[j] = 0.2;
...
int slave(int rank) 
{
  ...
  double start_wtime = MPI_Wtime();   // wall time
  int proposal_count = 0;
  int emu_count = 0;

  // --- Open progress file inside output directory ---
  ...
  // --- Ensure parameter mapping is done so we know number of cosmological parameters ---
  if (!mapping_done) {
      error *err = initError();
      if (initialize_clik_objects(&err)) {
          // Count parameters with USAGE_CAMB
          g_n_cosmo = 0;
          for (int i = 0; i < global_config.param_count; i++) {
              if (global_config.params[i].usage == USAGE_CAMB) {
                  g_n_cosmo++;
              }
          }
          if (g_n_cosmo == 0) {
              fprintf(stderr, "Rank %d: WARNING: No cosmological parameters found! Using default 6.\n", rank);
              g_n_cosmo = 6; // fallback
          } else {
              printf("Rank %d: Found %d cosmological parameters for emulator.\n", rank, g_n_cosmo);
          }
      } else {
          fprintf(stderr, "Rank %d: Failed to initialize clik for mapping. Using default 6 cosmological parameters.\n", rank);
          g_n_cosmo = 6;
      }
      mapping_done = 1;
  }

  // --- INITIALISE EMULATOR (once per slave, only if USE_EMULATOR=1) ---
  if (USE_EMULATOR && g_emulator == NULL) {
      ...
      // Print table header once per slave
      printf("\n"
             "------------------------------------------------------------------------------------------------------------------------------\n"
  ...
  for (;;) {
      ...
      // --- Extract cosmological parameters (only up to g_n_cosmo, but we assume first g_n_cosmo are cosmo) ---
      double cosmo_params[g_n_cosmo];
      for (int i = 0; i < g_n_cosmo; i++) {
          cosmo_params[i] = task[i];
      }

      // --- Try emulator prediction (if enabled and ready) ---
      int use_emu = 0;
      int emu_success = 0;
      double uncertainty = 0.0;
      double emu_cl_tt[2601], emu_cl_te[2601], emu_cl_ee[2601], emu_cl_bb[2601];

      if (USE_EMULATOR && g_emulator && emulator_is_ready(g_emulator)) {
          emu_success = emulator_predict(g_emulator, cosmo_params,
                                         emu_cl_tt, emu_cl_te, emu_cl_ee, emu_cl_bb,
                                         &uncertainty);
          if (emu_success && uncertainty <= FALLBACK_THRESHOLD) {
              use_emu = 1;
              Chi2X = run_plc(rank, task, emu_cl_tt, emu_cl_te, emu_cl_ee, emu_cl_bb);
          }
      }

      // --- If emulator not used, fallback to true model ---
      if (!use_emu) {
          // Allocate fresh arrays for this evaluation
          double *Cl_TT = (double*)malloc(2601 * sizeof(double));
          double *Cl_TE = (double*)malloc(2601 * sizeof(double));
          double *Cl_EE = (double*)malloc(2601 * sizeof(double));
          double *Cl_BB = (double*)malloc(2601 * sizeof(double));

          if (!Cl_TT || !Cl_TE || !Cl_EE || !Cl_BB) {
              fprintf(stderr, "Rank %d: Memory allocation failed for fallback.\n", rank);
              Chi2X = 1e10;
              ...
              if (magicnum1 != 0) {
                  ...
                  // Update emulator buffer with true spectra (if emulator exists and USE_EMULATOR=1)
                  if (USE_EMULATOR && g_emulator) {
                      double true_logL = -0.5 * Chi2X;
                      emulator_update(g_emulator, local_step++, cosmo_params,
                                      Cl_TT, Cl_TE, Cl_EE, Cl_BB, true_logL);
                      // DO NOT FREE – buffer now owns the pointers.
                  } else {
                      // No emulator – free arrays
                      free(Cl_TT); free(Cl_TE); free(Cl_EE); free(Cl_BB);
                  }
              } else {
                  Chi2X = 1e10;
      ...
      // --- Update counters and log every 5000 proposals ---
      proposal_count++;
      if (use_emu) emu_count++;
      if (proposal_count % 500 == 0) {
          double now = MPI_Wtime();
          double elapsed = now - start_wtime;
      ...
      double chi2 = Chi2X;   // raw chi2 (positive)
      Chi2X = -Chi2X;        // now negative for likelihood combination

      // --- Combine likelihoods (unchanged) ---
      if (WMAP7 == 1)
      ...
      // ============================================================
      // ** CLEAN TABLE PRINT – one line per evaluation **
      // ============================================================
      double logL = -0.5 * chi2;   // log-likelihood
      int max_print = (g_n_cosmo < 6) ? g_n_cosmo : 6;
      printf(" %2d     %6d    %1d      %1d     %8.4e   %8.2f %8.2f   ",
             rank, mcmc_step, use_emu, (!use_emu), uncertainty, chi2, logL);
      for (int i = 0; i < max_print; i++) {
          printf("%8.4f ", cosmo_params[i]);
      }
      ...
      // Send result to master (via middleman)
      if (middleman(rank) != 1)
          MPI_Send(task, TASKARRAY_SIZE, MPI_DOUBLE, mymiddlemann(rank), TAKERESULT, MPI_COMM_WORLD);

      // Middleman logic: if DR is off, only the first proposal (middleman's own) is considered.
      take = 1;
      takenindex = 0;
      task[TAKEPOS] = 0;
      ...
      if (middleman(rank) == 1) {
          for (int ii = rank; ii < rank + SLAVEPARCHAIN; ii++) {
              // If DR off, ignore all but the first slave
              if (!USE_DR && ii != rank) {
                  // still receive to avoid deadlock (already received above)
                  continue;
              }
              take = 1;
              if (ii != rank)
  ...
  // This point is never reached if the loop is infinite,
...
// Add this function somewhere before main() (e.g., after the include block)
...
//   const char *outdir = ".";   // default output directory
...
//   // Create output directory if it doesn't exist (master only)
//   if (myrank == 0) {
//       char mkdir_cmd[512];
...
//   if (myrank == 0) {
//       printf("\nRestart = %d", Restart);
//       master(startnum, Restart, outdir);
//   } else {
