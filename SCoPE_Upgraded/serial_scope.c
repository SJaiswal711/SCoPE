 ...
 * accepted points are written to the chain file.
 * Each line: step mult logL param_0 ... param_N
 */
...
int main(int argc, char **argv)
{
    ...
    /* Open output file */
    FILE *chain_file = fopen("chain_serial2.txt", "w");
    if (!chain_file) {
        fprintf(stderr, "Error: cannot open chain file\n");
        exit(1);
    }

    /* Write header: step, mult, logL, then parameters */
    ...
    /* Multiplicity counter for current point */
    int multiplicity = 1;   // initial point counted as 1
    ...
    for (int step = 0; step < max_steps; step++) {
        ...
        do {
            ...
            if (log_ratio > 0 || exp(log_ratio) > (rand()/(double)RAND_MAX)) {
                accepted = 1;
                /* --- Write previous point with its multiplicity before moving --- */
                fprintf(chain_file, "%d %d %.12e", step, multiplicity, current_loglike);
                for (int i = 0; i < PARAMETERS; i++)
                ...
                chain[chainBack++] = t;
                chainSize++;
                multiplicity = 1;   // reset for new point
        ...
        /* Progress output */
        if (step % 10 == 0) {
            printf("Step %d, logL = %.2f, factor = %.4f, acc = %d, acc_rate = %.2f, rej = %d, mult=%d\n",
                   step, current_loglike, EntireFactor, accepted, acc_rate, consecutive_rejections, multiplicity);
            fflush(stdout);
        }
    }
    ...
    fclose(chain_file);
    printf("Chain written to chain_serial2.txt\n");
