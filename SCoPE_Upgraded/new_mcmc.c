...
int throwDice(Task chain, Task *next, MultiGaussian *MG, int scale)
{
  ...
  for (unsigned int i = 0; i < PARAMETERS; i++)
  {
    ...
    /* === DR FIX 1: store the standardized eigenbasis displacement ===
       e_i = gamma * z_i, where z_i ~ N(0,1).
       (Old line stored randomq = sqrt(lambda) * z, which is not the
       quantity the DR q-ratio needs.) */
...
int throwSlowDice(Task chain_pt, Task *next, MultiGaussian *MGs, int scale) {
    ...
    for (unsigned int k = 0; k < MGs->SIZE; k++) {
        ...
        /* === DR FIX 2: store standardized slow-eigenbasis displacement ===
           e_k = gamma * z_k, k = 0..N_SLOW-1, stored compactly in
           RANDPOS + k. Fast slots zeroed below. */
    ...
    /* zero out the unused fast RANDPOS slots for cleanliness */
...
int slave(int rank) {
    ...
    for (;;) {
        ...
        /* === DR FIX 3: full recursive parallel DR (Scheme B) ===
           Proposals y_1..y_K all come from y0, each at a progressively
           shrinking scale gamma_m = drfactor^(m-1). The middleman builds
           the recursive acceptance matrix A[i][j] = alpha_i(y_j, ...) and
           tests each stage sequentially. q-ratios are computed from the
           standardized eigenbasis displacements stored in RANDPOS. */
        ...
        /* number of eigenbasis dims used in the q-ratio */
        int dr_dim = (int)task_own[DRAGPOS] == 1 ? N_SLOW : PARAMETERS;

        /* --- caches for the DR recursion --- */
        ...
        /* --- Stage 1 (middleman's own proposal) --- */
        ...
        if (un_stage[1] < A_dr[1][0]) {
            takenindex = 1;
            accepted_stage = 0;               /* 0-based; stage 1 */
            ...
            for (int stage = 2; stage <= Kmax; stage++) {
                ...
                /* A_dr[stage][0] — the acceptance actually tested */
                ...
                for (int mp = 1; mp < stage; mp++) {
                    ...
                    for (int k = 0; k < dr_dim; k++) {
                        ...
                        double diff_0       = u_stage[mp][k];   /* u_0 = 0 */
                ...
                if (un_stage[stage] < A_dr[stage][0]) {
                    takenindex = 1;
                    accepted_stage = stage - 1;      /* 0-based */
