...
typedef struct {
    ...
    int likelihood_count;
     ...
     int cosmological_sample; // tells if we are sampling only cosmological parameters(1) or also nuisance parameters(0)
...
static void set_planck_nuisance_defaults(Config *config)
{
    const char *names[] =
    {
        "A_ps_100", "A_ps_143", "A_ps_217",
        "A_cib_143", "A_cib_217", "A_sz",
        "r_ps", "r_cib", "n_Dl_cib",
        "cal_100", "cal_217",
        "xi_sz_cib", "A_ksz", "Bm_1_1"
    };

    // Planck 2015 plik TT best-fit, arXiv:1507.02704 Table 4
    double values[] =
    {
        211.0, 44.0, 96.0,
        3.6,   52.0, 6.9,
    ...
    for(int i = 0; i < N; i++)
    {
        ...
        ParameterConfig *target = NULL;
        for(int j = 0; j < config->param_count; j++)
        {
            if(strcmp(config->params[j].name, names[i]) == 0)
            {
                target = &config->params[j];
                printf("  [FIXED] Overwriting '%s' with Planck default: %.4f\n",
                       names[i], values[i]);
                break;
            }
        }
        ...
        if(target == NULL)
        {
            ...
            {
                fprintf(stderr, "  [ERROR] MAX_PARAMS reached, cannot add '%s'\n", names[i]);
                continue;
            }
            target = &config->params[config->param_count++];
            strncpy(target->name, names[i], MAX_PARAM_NAME_LENGTH - 1);
            target->name[MAX_PARAM_NAME_LENGTH - 1] = '\0';
            printf("  [FIXED] Adding '%s' with Planck default: %.4f\n",
                   names[i], values[i]);
        }

        // Fix the parameter — all three bounds collapsed to the default value
        target->lower_bound = values[i];
        target->upper_bound = values[i];
        target->sigma       = 0.0;
        target->is_estimated = 0;
        target->usage       = USAGE_UNKNOWN;
...
//     config->cosmological_sample = 0; // default to 0 (sample both cosmological and nuisance parameters)
    
//     char line[MAX_LINE_LENGTH];

//     if(strncmp(trimmed,"cosmological_sample",20)==0)
// {
//     sscanf(strchr(trimmed,'=')+1,"%d",
//            &config->cosmological_sample);
//     continue;
// }
...
//     if(config->cosmological_sample)
// {
//     set_planck_nuisance_defaults(config);
// }
//     fclose(file);
//     return 1;
// }
...
static int load_config(const char* filename, Config* config) {
    ...
    config->param_count = 0;
    config->likelihood_count = 0;
    config->cosmological_sample = 0; // default: sample both cosmo + nuisance
    ...
    while(fgets(line, sizeof(line), file)) {
        ...
        // ✅ FIX: cosmological_sample check is now INSIDE the loop
        if(strncmp(trimmed, "cosmological_sample", 19) == 0) {
            char* eq = strchr(trimmed, '=');
            if(eq) {
                sscanf(eq + 1, "%d", &config->cosmological_sample);
                printf("  [CONFIG] cosmological_sample = %d\n",
                       config->cosmological_sample);
            }
            continue; // ✅ valid here — skip to next line
        }

        // Skip CAMB path
        if(strstr(trimmed, "CAMB_path") != NULL) continue;

        // Parse likelihood paths
        if(strstr(trimmed, "Likelihood_Path_") != NULL) {
            char* equals_pos = strchr(trimmed, '=');
            if(equals_pos && config->likelihood_count < 10) {
                strncpy(config->likelihood_paths[config->likelihood_count],
                       trim(equals_pos + 1),
                       sizeof(config->likelihood_paths[0]) - 1);
                config->likelihood_count++;
            }
            continue;
        }

        // Parse all other parameters (cosmological + nuisance)
    ...
    // ✅ Now cosmological_sample is correctly set before this check
    if(config->cosmological_sample == 1) {
        printf("  [INFO] cosmological_sample=1: fixing nuisance params to Planck defaults.\n");
        set_planck_nuisance_defaults(config);
    } else {
        printf("  [INFO] cosmological_sample=0: nuisance params will be sampled from param.ini.\n");
    }
